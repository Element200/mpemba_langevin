...
potential = mpemba.special_potentials.AsymmetricDoubleWellPotential(x_min=-6, x_max=6, E_barrier=2, E_tilt=0.4, force_asymmetry=0.5)
...
def analytical_distances_to_boltzmann(p_trajectories, potential, distance_function=mpemba.distance_functions.L1, axis=1, k_BT_b=1, x=None, boltzmann=None):
    """
...
def propagate_initial_state(ensemble, plot=False, resolution=1000, D=85, dt=1e-7, error_tolerance=5e-2, saving_timestep=100, colors=['r','b','k']):
    # ensemble.plot_distances(use_true_distro=False, colors=colors)
    interpolate_0 = scipy.interpolate.interp1d(ensemble.pdfs.x, ensemble.pdfs.loc[ensemble.temperatures[0],:,0], kind='cubic')
    interpolate_1 = scipy.interpolate.interp1d(ensemble.pdfs.x, ensemble.pdfs.loc[ensemble.temperatures[1],:,0], kind='cubic')
    x_fine = np.linspace(ensemble.pdfs.x.to_numpy().min(), ensemble.pdfs.x.to_numpy().max(), resolution)
    dx = x_fine[1]-x_fine[0]
    p_0 = fokker_planck_solver(interpolate_0(x_fine)/(interpolate_0(x_fine).sum()*dx), potential, D=D, t_max=float(ensemble.times.max().data), dt=dt, error_tolerance=error_tolerance, x=x_fine, saving_timestep=saving_timestep)
    p_1 = fokker_planck_solver(interpolate_1(x_fine)/(interpolate_1(x_fine).sum()*dx), potential, D=D, t_max=float(ensemble.times.max().data), dt=dt, error_tolerance=error_tolerance, x=x_fine, saving_timestep=saving_timestep)
    
    overlay_distances = analytical_distances_to_boltzmann(np.array([p_0, p_1]), potential, axis=1, x=x_fine)
    if plot:
        ensemble.plot_distances(use_true_distro=True, colors=colors)
        plt.loglog(np.arange(overlay_distances.shape[1])*1e-5, overlay_distances[0,:], f'{colors[0]}--')
        plt.loglog(np.arange(overlay_distances.shape[1])*1e-5, overlay_distances[1,:], f'{colors[1]}--')
        plt.ylim((1e-2,2))
        plt.xlabel("t")
        plt.ylabel("Distance")
    return p_0, p_1, overlay_distances
