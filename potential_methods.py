...
def TimeDependentPotential(cls):
    class TimeDependentPotential_Methods(cls):
        def __init__(self, *args, **kwargs):
            ...
            x, t = sym.symbols('x t') # Define symbols for numeric computation
            self.F_0 = sym.lambdify((x, t), sym.diff(-self.U_0(x, t), x)) # Precompute the derivative and store it as a lambda function. Precomputing is very important for speed. Note that lambdify changes all sympy functions into their numpy counterparts
            self._U_cache = None
        ...
        def U(self, x, t):
            ...
            if self._U_cache is None:
                x_sym, t_sym = sym.symbols('x t') # Define symbols for numeric computation
                self._U_cache = sym.lambdify((x_sym,t_sym), self.U_0(x_sym, t_sym))
            return self._U_cache(x,t)
            # _U_cache is necessary to make sure we're not re-lambdifying U_0 every time U is called (slow AF).
        def F(self, x, t):
            """
            ...
            """
            return self.F_0(x, t) # F_0 is already lambdified so no need to do it again
        def plot_potential(self, plot_range=None, t=0):
            """
            Plot the potential.
            ...
            plt.plot(plot_range, self.U(plot_range, t), 'g')
            return None
        def plot_force(self, plot_range=None, t=0):
            """
            Plot the force.
            ...
            if plot_range is None:
                plot_range = np.linspace(self.x_min, self.x_max, 50)
            plt.plot(plot_range, self.F(plot_range, t), 'c')
            return None
        def _boltzmann_unnormalised(self, x, k_BT, t=0):
            # unnormalised, unvectorised boltzmann distro at time zero
            return np.exp(-self.U(x, t)/k_BT)
            # If k_BT is array-like, generate an array of shape len(x),len(k_BT)
        def boltzmann(self, x, k_BT, integration_bounds=None, t=0):
            """
            ...
            Z = scipy.integrate.quad_vec(lambda x: self._boltzmann_unnormalised(x, k_BT, t=t), *integration_bounds)[0]
            func = np.vectorize(lambda x, k_BT: self._boltzmann_unnormalised(x, k_BT, t=t)/Z) # Normalise the PDF
            return func(x, k_BT)
    ...
    return TimeDependentPotential_Methods

def Potential(cls):
    """
    Convert a class defining a potential into an object that we can get a bunch of useful things like the force, boltzmann distribution, etc from. This has a generic form and does not add the piecewise conditions that are useful in experiments.

    Returns
    -------
    object
        Child class of cls with helpful attributes.

    """
    class Potential_Methods(cls):
...
def BoundedForcePotential(child_cls):
    ...
    -------
    object
        Grandchild class of cls with helpful attributes.

    """
    # child_cls = Potential(cls) # Potential is a child class of cls
    class BoundedForcePotential_Methods(child_cls):
        """Takes in a functional form of a potential and defines a bunch of handy functions."""
        
        def __init__(self, *args, **kwargs):
            # cls.__init__(self, *args, **kwargs) # Initialise the parent class so we can use the variables defined there. args and kwargs are stuff (eg. relevant parameters) to be passed up to the parent class.
            child_cls.__init__(self, *args, **kwargs)
            x = sym.symbols('x') # Define symbols for numeric computation
            self.F_0 = sym.lambdify(x, sym.diff(-self.U_0(x))) # Precompute the derivative and store it as a lambda function. Precomputing is very important for speed.
