...
def Potential(cls):
    ...
    class Potential_Methods(cls):
        ...
        def __repr__(self):
            """
            Display useful information about the object when calling it with no arguments.

            Returns
            -------
            str
                Useful information about the potential (for now, just the form of U_0).

            """
            x = sym.symbols('x')
            return str(self.U_0(x))
    ...
    class BoundedForcePotential_Methods(child_cls):
        ...
        def __init__(self, *args, **kwargs):
            ...
            def _newton_raphson_solver(f, x_0, max_iterations = 100, dx = 1e-6, return_error=False, tolerance = 1e-5, debug_mode=False):
                ...
                return x_n
            x = sym.symbols('x') # Define symbols for symbolic computation
        ...
        def __repr__(self):
            ...
            """
            x = sym.symbols('x')
            U = sym.Piecewise((self.U_0(x), (x>=self.x_l) & (x <= self.x_r)),
                              (self.U_0(self.x_l)-self.F_left*(x-self.x_l), x<=self.x_l),
                              (self.U_0(self.x_r)+self.F_right*(x-self.x_r), x>=self.x_r))
            # print(U)
            return "Bounded force potential"
